...
C 	x = 0.01 + float(ix)*0.025
C         write(14,*)x,f2p_b(x,q2),f2_wtl(1,x,Q2),f2_ctq(1,x,q2)  ! proton
C         write(15,*)x,f2n_b(x,q2),f2_ctq(0,x,q2)                  ! neutron
C	enddo
C        end
...
**********************************************************************
	...
	double precision function f2_ctq(in,x,q2)
***********************************************
*       in = 1 - proton
*       in = 0 - neutron
*
*     The range of (x, Q) used in this round of global analysis is, approxi-
*     mately,  0.01 < x < 0.75 ; and 4 GeV^2 < Q^2 < 400 GeV^2 for fixed target
*     experiments and 0.0001 < x < 0.1 from HERA data.
*
***********************************************
	double precision, intent(in) :: x,q2
	integer, intent(in) :: in
	dimension pdf(-6:6)
	double precision xx,qq,pdf
        f2_ctq = 0.0
        if(x.ge.1.0)return
	XX = x
	q = sqrt(q2)
	QQ = q
	Iset = 1
	call Ctq3Pds(Iset,Pdf,XX,QQ,Irt)
	u  = Pdf(1) !+ Pdf(-1)
	ub = Pdf(-1)
	d  = Pdf(2) !+ Pdf(-2)
	db = Pdf(-2)
	s  = Pdf(3)
	sb = Pdf(-3)
	if(in.eq.1)f2 = (4.0*(u+ub)+(d+db)+(s+sb))/9.0
	if(in.eq.0)f2 = (4.0*(d+db)+(u+ub)+(s+sb))/9.0
	f2_ctq = f2
	return
	end
